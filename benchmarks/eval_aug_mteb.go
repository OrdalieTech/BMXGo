...
func eval_aug_mteb() {
	if len(os.Args) != 3 {
		fmt.Println("usage: go run eval_aug_mteb.go model dataset")
		os.Exit(1)
	}
	...
	// Search with augmented queries
	fmt.Println("Searching with augmented queries...")
	startTime := time.Now()
	for _, query := range queries {
		// Generate augmented queries
		augmentedQueries, err := build.GenerateAugmentedQueries(query, 3)
		if err != nil {
			fmt.Printf("Error generating augmented queries: %v\n", err)
			continue
		}

		// Search with original and augmented queries
		rankedDocs := bmxAdapter.SearchAugmented(query, augmentedQueries, 5)
		fmt.Printf("Query: %s\n", query)
