...
# Python type for map map[string]map[string]float64
class Map_string_Map_string_float64(go.GoClass):
	""""""
	def __init__(self, *args, **kwargs):
		"""
		handle=A Go-side object is always initialized with an explicit handle=arg
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _BMXGo.Map_string_Map_string_float64_CTor()
			_BMXGo.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], _collections_abc.Mapping):
					raise TypeError('Map_string_Map_string_float64.__init__ takes a mapping as argument')
				for k, v in args[0].items():
					_BMXGo.Map_string_Map_string_float64_set(self.handle, k, v)
	def __del__(self):
		_BMXGo.DecRef(self.handle)
	def __str__(self):
		s = 'BMXGo.Map_string_Map_string_float64 len: ' + str(len(self)) + ' handle: ' + str(self.handle) + ' {'
		if len(self) < 120:
			for k, v in self.items():
				s += str(k) + '=' + str(v) + ', '
		return s + '}'
	def __repr__(self):
		s = 'BMXGo.Map_string_Map_string_float64({'
		for k, v in self.items():
			s += str(k) + '=' + str(v) + ', '
		return s + '})'
	def __len__(self):
		return _BMXGo.Map_string_Map_string_float64_len(self.handle)
	def __getitem__(self, key):
		return Map_string_float64(handle=_BMXGo.Map_string_Map_string_float64_elem(self.handle, key))
	def __setitem__(self, key, value):
		_BMXGo.Map_string_Map_string_float64_set(self.handle, key, value.handle)
	def __delitem__(self, key):
		return _BMXGo.Map_string_Map_string_float64_delete(self.handle, key)
	def keys(self):
		return go.Slice_string(handle=_BMXGo.Map_string_Map_string_float64_keys(self.handle))
	def values(self):
		vls = []
		kys = self.keys()
		for k in kys:
	...
	def __iter__(self):
		return iter(self.items())
	def __contains__(self, key):
		return _BMXGo.Map_string_Map_string_float64_contains(self.handle, key)
...
class Query(go.GoClass):
	...
	@property
	def F_table(self):
		return Map_string_Map_string_float64(handle=_BMXGo.BMXGo_Query_F_table_Get(self.handle))
	@F_table.setter
	def F_table(self, value):
		if isinstance(value, go.GoClass):
			_BMXGo.BMXGo_Query_F_table_Set(self.handle, value.handle)
		else:
			raise TypeError("supplied argument type {t} is not a go.GoClass".format(t=type(value)))
	@property
	def IDF_table(self):
		return Map_string_float64(handle=_BMXGo.BMXGo_Query_IDF_table_Get(self.handle))
	@IDF_table.setter
	def IDF_table(self, value):
		if isinstance(value, go.GoClass):
			_BMXGo.BMXGo_Query_IDF_table_Set(self.handle, value.handle)
		else:
			raise TypeError("supplied argument type {t} is not a go.GoClass".format(t=type(value)))
	@property
	def E_tilde_table(self):
		return Map_string_float64(handle=_BMXGo.BMXGo_Query_E_tilde_table_Get(self.handle))
	@E_tilde_table.setter
	def E_tilde_table(self, value):
		if isinstance(value, go.GoClass):
			_BMXGo.BMXGo_Query_E_tilde_table_Set(self.handle, value.handle)
		else:
			raise TypeError("supplied argument type {t} is not a go.GoClass".format(t=type(value)))
	@property
	def E_table(self):
		return Map_string_float64(handle=_BMXGo.BMXGo_Query_E_table_Get(self.handle))
	@E_table.setter
	def E_table(self, value):
		if isinstance(value, go.GoClass):
			_BMXGo.BMXGo_Query_E_table_Set(self.handle, value.handle)
		else:
			raise TypeError("supplied argument type {t} is not a go.GoClass".format(t=type(value)))
	@property
	def S_table(self):
	...
	def AugmentedWeights(self, value):
		if isinstance(value, go.GoClass):
			...
			raise TypeError("supplied argument type {t} is not a go.GoClass".format(t=type(value)))
	def IDF_table_fill(self, bmx, goRun=False):
		"""IDF_table_fill(object bmx) """
		_BMXGo.BMXGo_Query_IDF_table_fill(self.handle, bmx.handle, goRun)
	def F_table_fill(self, bmx, goRun=False):
		"""F_table_fill(object bmx) """
		_BMXGo.BMXGo_Query_F_table_fill(self.handle, bmx.handle, goRun)
	def E_tilde_table_fill(self, bmx, goRun=False):
		"""E_tilde_table_fill(object bmx) """
		_BMXGo.BMXGo_Query_E_tilde_table_fill(self.handle, bmx.handle, goRun)
	def E_table_fill(self, bmx, goRun=False):
		"""E_table_fill(object bmx) """
		_BMXGo.BMXGo_Query_E_table_fill(self.handle, bmx.handle, goRun)
...
class BMXAdapter(go.GoClass):
	...
	def __repr__(self):
		...
		for v in pr:
			if not callable(v[1]):
				sv += v[0] + '=' + str(v[1]) + ', '
		return sv + ')'
	def AddMany(self, ids, docs, goRun=False):
		"""AddMany([]str ids, []str docs) """
		_BMXGo.BMXGo_BMXAdapter_AddMany(self.handle, ids.handle, docs.handle, goRun)
	...
	def SearchAugmented(self, query, topK, num_augmented_queries):
		"""SearchAugmented(str query, int topK, int num_augmented_queries) object"""
		return SearchResults(handle=_BMXGo.BMXGo_BMXAdapter_SearchAugmented(self.handle, query, topK, num_augmented_queries))
	def SearchAugmentedMany(self, queries, topK, num_augmented_queries, maxConcurrent):
		"""SearchAugmentedMany([]str queries, int topK, int num_augmented_queries, int maxConcurrent) []object"""
		return Slice_BMXGo_SearchResults(handle=_BMXGo.BMXGo_BMXAdapter_SearchAugmentedMany(self.handle, queries.handle, topK, num_augmented_queries, maxConcurrent))
...
def E_tilde(F_table, qi, docs):
	"""E_tilde(object F_table, str qi, object docs) float
	
	Function to calculate E(qi)
	"""
	return _BMXGo.BMXGo_E_tilde(F_table.handle, qi, docs.handle)
def F(qi, doc):
	"""F(str qi, object doc) float
	
	Function to calculate F(qi, D)
	"""
	return _BMXGo.BMXGo_F(qi, doc.handle)
...
def IDF(qi, docs):
	"""IDF(str qi, object docs) float
	
	Function to calculate IDF
	"""
	return _BMXGo.BMXGo_IDF(qi, docs.handle)
