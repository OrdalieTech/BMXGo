...
ids_slice = go.Slice_string(ids[:2])

# Convert Python list of docs to Go slice
docs_slice = go.Slice_string(docs[:2])
...
for doc_id, doc in zip(ids, docs):
    print(doc)
    tokens = searcher.GetTokens(doc)
    print(f"Doc {doc_id}: {tokens}")  # Print first 20 tokens

# Print query tokens
print("\nQuery tokens:")
for query_id, query in zip(query_ids, queries):
    tokens = searcher.GetTokens(query)
    print(f"Query {query_id}: {tokens}")  # Print first 20 tokens
...
results = searcher.SearchAugmentedMany(queries_slice, topk, 3, 50)
